 '''
리스트(List)
: 대괄호 [] 안에 콤마로 항목을 구분하여, 대입연산자로 항목을 변경할 수 
있는 시퀀스 자료형이다. 서로 다른 자료형의 항목으로 구성할 수 있고, 
...
#출력
print('list1', list1) # 리스트명을 사용하면 전체 항목이 출력됨 
# 인덱스를 통해 출력 가능 
...
# 리스트 간의 연결 가능. 리스트내에 또 다른 리스트가 삽입된다. 
print(f"{'리스트 연결':-^30}")
...
# 리스트의 마지막 항목 삭제 및 반환 
print(list1.pop()) # 삭제된 항목 반환. 즉 삭제와 동시에 출력가능.  
