# SQLite모듈은 기본으로 제공되므로 import만 하면 사용 가능 
import sqlite3
import random 

# sqlite 연결 및 DB파일 생성 
conn = sqlite3.connect('./saveFiles/biography.db')  
# 커서 생성. 이를 통해 여러가지 DB작업을 할 수 있다. 
curs = conn.cursor()  
# 입력값 구분을 위해 난수 생성 
...
try:
    ...
    '''테이블이 이미 생성된 상태라면 예외가 발생하게 되므로 except절에서
    처리해야한다. 혹은 if not exist를 쿼리문에 추가해도 된다. '''
    print("[예외발생]테이블은 이미 생성되었습니다.", e)

# 레코드 1개 삽입. 튜플을 사용하여 인파라미터를 설정함. 
curs.execute('insert into people values (?,?,?)', (f'이순신{rnd}','장군',500))

# 레코드 2개 이상 삽입. 튜플로 구성된 리스트 사용함. 
# 이때는 executemany() 함수 사용. 
...
rows = [[f'곽재우{rnd}','의병',1100], 
        [f'유성룡{rnd}','재상',1200], 
        [f'임꺽정{rnd}','의적',1500]]
# 입력데이터로 사용할 리스트의 크기만큼 반복해서 쿼리문 실행 
for row in rows :
